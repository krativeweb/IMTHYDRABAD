...
import { parseHTML } from 'linkedom';   // <-- correct import

const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL;

/* -------------------------------------------------
   Tiny HTML parser that works on server & client
   ------------------------------------------------- */
const safeParseHTML = (html) => {
  ...
  // Node / Vercel – linkedom
  const { document } = parseHTML(html);
  return document;
};

/* -------------------------------------------------
   Helper: extract <li> text
   ------------------------------------------------- */
...
/* -------------------------------------------------
   Education + Service (both in one block)
   ------------------------------------------------- */
const parseEducationAndService = (html) => {
  ...
  srvList?.querySelectorAll('li').forEach(li => {
    ...
    if (m) {
      service.push({ years: m[1], role: m[2], institution: m[3] });
    }
  });
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        .social-icon.rounded-0{border-radius:0}
        ...
        <div className="container my-5">
          <div className="faculty-tabs mt-4">
            <ul className="nav nav-pills justify-content-center flex-wrap gap-2 mb-4" id="facultyTab" role="tablist">
              {[
                'Brief','Education','Teaching & Research Interests','Publications',
                'Awards & Honors','Service','Other Professional Activities'
              ].map((tab, i) => (
                ...
                <ul className="list-group list-group-flush mt-3">
                  {faculty.education.map((e, i) => (
                    <li className="list-group-item" key={i}>
                      <strong>{e.degree}</strong> – {e.institution} ({e.years})
                    </li>
                  ))}
                </ul>
              </div>
